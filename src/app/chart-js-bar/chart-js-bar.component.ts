...
@Component({
  ...
  styleUrls: ['./chart-js-bar.css']
})
export class ChartJSBarComponent implements OnInit, OnDestroy, AfterViewInit {
  showdata = false;
  ...
  updateData(x: SliderInfo) {
    ...
    this.d2CI.title = this.Report1.ReportName + ' as of ' + this.Report1.DateOfReference;
    this.titleForCard1 = 'Cases in Bar format as of ' + this.Report1.DateOfReference + ',1';
    setTimeout(() => {
      this.OAS.readJSONToText(fileURL1, this.d2CI.title, 'Line', 1, 'CASES',
